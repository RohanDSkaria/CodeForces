...
bool cc(const pair<int, int>& a, const pair<int, int>& b) {
    return min(2*a.fi,a.se)-min(a.fi,a.se) > min(2*b.fi,b.se)-min(b.fi,b.se);
}
void solve(){
    int n,f,ans=0;cin>>n>>f;
    vpii a(n);
    for(auto& [x,y]:a) cin>>x>>y;
    // for(int i=0; i<n; i++){cin>>a[i].fi>>a[i].se;}
    sort(all(a),cc);
    for(int i=0; i<n; i++) ans+=(i<f)?(min(2*a[i].fi, a[i].se)):(min(a[i].fi,a[i].se));
    cout<<ans<<endl;
}
int32_t main(){
    IOS
