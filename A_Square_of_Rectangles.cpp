...
bool solve(){
    ...
    if(a[x][y]==a[0][y]+a[1][y]+a[2][y]-a[x][y]) return a[x][y]==(a[x][!y]+a[(x+1)%3][!y]) && a[(x+1)%3][!y]==a[(x+2)%3][!y];
    if(a[x][y]==a[(x+1)%3][y] && a[(x+1)%3][y]==a[(x+2)%3][y]) return a[x][y]==(a[0][!y]+a[1][!y]+a[2][!y]);
    return 0;
}
int32_t main(){
    IOS int t=1;
